...
async function loadProduce(page = 1) {
    ...
    try {
        ...
        let url =
            `${PRODUCE_API_BASE}/produce?page=${currentPage}&limit=${pageLimit}`;

        // If search is active, use search API
        if (currentSearch.trim() !== "") {

            url =
                `${PRODUCE_API_BASE}/produce/search` +
        ...
        if (!response.ok) {
            throw new Error(
                data.message || "Unable to load produce."
            );
        }
...
async function loadProduceDetails(produceId) {
    ...
    try {
        ...
        const response = await fetch(
            `${PRODUCE_API_BASE}/produce/${encodeURIComponent(produceId)}`,
            {
                method: "GET",
                credentials: "include",
                headers: {
        ...
        if (!response.ok) {
            ...
            throw new Error(
                data.message || "Unable to load produce details."
            );
